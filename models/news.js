...
exports.selectArticles = (sort_by = "created_at", order = "desc", topic) => {
  ...
  return db
    .query(
      `SELECT users.username AS author, title, articles.article_id, topic, articles.created_at, articles.votes, count(comment_id)::INT AS comment_count FROM articles INNER JOIN users ON articles.author = users.username AND articles.topic = mitch LEFT JOIN comments ON articles.article_id = comments.article_id   GROUP BY articles.article_id, users.username ORDER BY ${sort_by} ${order};`
    )
    .then((result) => {
